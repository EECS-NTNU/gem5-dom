...
template<class Impl>
DeltaPred<Impl>::DeltaPred(O3CPU *_cpu, const DerivO3CPUParams &params)
    : BaseAddPred<Impl>(_cpu, params), stats(_cpu)
{
}

template<class Impl>
Addr
DeltaPred<Impl>::predictFromPC(Addr pc, int runAhead) {
...
template<class Impl>
void
DeltaPred<Impl>::updatePredictor(Addr realAddr, Addr pc,
                                   InstSeqNum seqNum,
                                   int packetSize)
{
...
template<class Impl>
int
DeltaPred<Impl>::getPacketSize(Addr pc) {
    auto it = entries.find(pc);
    if (it == entries.end()) return 0;
    struct AddrHistory* entry = it->second;
...
template<class Impl>
Addr
DeltaPred<Impl>::walkPred(Addr pc, int patternSize, int steps) {
...
template<class Impl>
int
DeltaPred<Impl>::longestMatchingPattern(Addr pc) {
...
template<class Impl>
bool
DeltaPred<Impl>::matchSize(Addr pc, int size) {
    int pattern[size];
    struct AddrHistory* entry = entries.find(pc)->second;
    if (entry->strideHistory.size() < deltaHistory)
        return false;
...
template<class Impl>
const std::string
DeltaPred<Impl>::name() const
{
    return "delta_predictor";
}

template<class Impl>
DeltaPred<Impl>::DeltaPredStats::DeltaPredStats(Stats::Group *parent)
    : Stats::Group(parent),
