...
class ADD_PRED
{
    ...
    std::map<uint64_t, Prediction*> next_fetches;
    ...
    uint16_t update_sig_l1(uint16_t old_sig, int delta)
    {
        ...
        // since we need to track deltas from +63 to -63
    ...
    void check_for_stream_l1(int index, uint64_t cl_addr, uint8_t cpu)
    {
        ...
        { // stream is detected
            ...
            // stream is classified as strong
            // if more than 3/4th entries belong to stream
    ...
    void prefetch_line(uint64_t ip, uint64_t prefetch, uint64_t seqNum)
    {
        if (next_fetches.find(ip) != next_fetches.end()) {
            if (!(seqNum == next_fetches[ip]->seqNum)) {
                ...
                seqNum,
                ip);
                next_fetches[ip]->prefetches.clear();
                next_fetches[ip]->seqNum = seqNum;
            }
        } else {
            DPRINTF(AddrPredDebug,"Made new prediction line "
            " with seqNum [sn:%llu]\n", seqNum);
            next_fetches[ip] = new Prediction();
            next_fetches[ip]->seqNum = seqNum;
        }
        next_fetches[ip]->prefetches.push_back(prefetch);
    }

    public:
    uint64_t predictLoad(uint64_t ip, int runahead)
    {
    ...
    void updatePredictor(uint64_t addr, uint64_t ip, uint64_t seqNum)
    {
        ...
        { // new/conflict IP
            DPRINTF(AddrPredDebug, "New IP encountered\n");
            ...
            prefetch_line(ip, pf_address, seqNum);
            return;
        }
        else
        ...
        { // stream IP
            ...
            DPRINTF(AddrPredDebug, "Discovered stream\n");
            for (int i = 0; i < prefetch_degree; i++)
            {
                ...
                SIG_DP(cout << "1, ");
                 ...
                 trackers_l1[cpu][index].last_stride != 0)
        { // CS IP
            DPRINTF(AddrPredDebug, "Stride discovered: %d\n",
                trackers_l1[cpu][index].last_stride);
            for (int i = 0; i < prefetch_degree; i++)
            {
                uint64_t pf_address = (cl_addr +
                        (trackers_l1[cpu][index].last_stride * (i + 1)))
                    << LOG2_BLOCK_SIZE;
                 ...
                 DPT_l1[cpu][signature].delta != 0)
        {
            ...
            for (i = 0; i < prefetch_degree; i++)
            {
                ...
                // we are not prefetching at L2
                // for CPLX type, so encode delta as 0
                if (DPT_l1[cpu][signature].conf > 0)
                { // prefetch only when conf>0 for CPLX
                    DPRINTF(AddrPredDebug,
                            "Delta confidence found for iteration %d\n", i);
                    prefetch_line(ip, pf_address, seqNum);
                    num_prefs++;
                    SIG_DP(cout << pref_offset << ", ");
                }
                signature = update_sig_l1(signature,
